...
import { Avatar, Button, Space, Spin, Table, Tooltip } from "antd";
...
import { deleteStart, getListStart } from "./redux";
import "./styles.scss";
    ...
    confirm({
      ...
      content: (
        <div>
          ...
          <strong> {user?.username} </strong> của người dùng{" "}
  ...
  const columns = [
    {
      title: "STT",
      render: (record) => (
        <div className="text-center">{listUser?.indexOf(record) + 1}</div>
      ),
      key: "id",
      width: "5%",
    },
    {
      ...
      render: (record) => (
        <div className="text-center">
          <Avatar size={64} src={BASE_URL + record} />
        </div>
      ),
      key: "avatar",
    },
    ...
    },
    {
      ...
      render: (record) => {
        switch (record) {
          case "ADMIN":
            return <div className="text-center">Quản trị viên</div>;
            break;
          case "USER":
            return <div className="text-center">Người dùng</div>;
            break;
          default:
