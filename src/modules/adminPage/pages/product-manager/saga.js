...
import { GetListProductApi, InsertProductApi, UpdateProductApi, DeleteProductApi } from '../../../../core/apis/productManager';
...
function* getListSaga(action) {
    try {
        const res = yield call(GetListProductApi, action.payload);
        if (res.ok) {
...
function* insertSaga(action) {
  try {
      const res = yield call(InsertProductApi, action.payload);
      if (res.ok) {
...
function* updateSaga(action) {
  try {
      const res = yield call(UpdateProductApi, action.payload);
      if (res.ok) {
...
function* deleteSaga(action) {
  try {
      const res = yield call(DeleteProductApi, action.payload);
      if (res.ok) {
...
export default function* ProductManagerSaga() {
