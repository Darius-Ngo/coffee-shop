...
import { GetListCategoryApi, InsertCategoryApi, UpdateCategoryApi, DeleteCategoryApi } from '../../../../core/apis/categoryManger';
...
function* getListSaga(action) {
    try {
        const res = yield call(GetListCategoryApi, action.payload);
        if (res.ok) {
...
function* insertSaga(action) {
  try {
      const res = yield call(InsertCategoryApi, action.payload);
      if (res.ok) {
          yield put({type: insertSuccess,payload: res.data,});
          toast.success('Thêm thông tin thành công', options);
          yield put({type: getListStart.type, payload: ''});
      } else {
...
function* updateSaga(action) {
  try {
      const res = yield call(UpdateCategoryApi, action.payload);
      if (res.ok) {
...
function* deleteSaga(action) {
  try {
      const res = yield call(DeleteCategoryApi, action.payload);
      if (res.ok) {
...
export default function* CategoryManagerSaga() {
